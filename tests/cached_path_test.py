...
class TestFileUtils(BaseTestCase):
    ...
    def test_cached_path_offline(self, monkeypatch):
        ...
        import cached_path  # We need the module `cached_path`, not the function with the same name.

        monkeypatch.setattr(cached_path, "_http_etag", mocked_http_etag)
...
class TestCacheFile(BaseTestCase):
...
class TestLocalCacheResource(BaseTestCase):
...
class TestCachedPathCommand(BaseTestCase):
    def test_local_file(self, capsys):
        sys.argv = ["cached-path", "--cache-dir", str(self.TEST_DIR), "LICENSE"]
        main()
        captured = capsys.readouterr()
        assert "LICENSE" in captured.out

    def test_inspect_empty_cache(self, capsys):
        sys.argv = ["cached-path", "--cache-dir", str(self.TEST_DIR), "--inspect"]
        main()
        captured = capsys.readouterr()
