...
CACHE_ROOT = Path(os.getenv("CACHED_PATH_CACHE_ROOT", Path.home() / ".cp"))
...
def _resource_to_filename(resource: str, etag: str = None) -> str:
    ...
    If `etag` is specified, append its hash to the resource's, delimited by a period.
    """
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cache remote files")
    parser.add_argument(
        "resources",
        type=str,
...
def _cached_path(args: argparse.Namespace) -> None:
